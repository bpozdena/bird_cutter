...
def parse_arguments():
    ...
    if args.example:
        ...
        print("python process_video.py 'path/to/your video.mp4' --output_dir 'path/to/output dir' --sensitivity 5.0 --buffer_before 3 --buffer_after 3 --min_interval 5 --min_avg_diff_sum 50000 --check_frame 30 --start_time 60.0 --end_time 180.0 --smoothing 30")
        exit(0)
...
def main():
    ...
    global motion_threshold  # Declare global to be used inside detect_motion
    video_path = args.video_path
    output_dir = args.output_dir or os.path.splitext(os.path.basename(video_path))[0]
    ...
    if check_frame_interval is not None:
        if check_frame_interval <= 0:
            print("Error: --check_frame must be a positive integer.")
            cap.release()
            return

        print(f"Checking differences every {check_frame_interval} frames...")
        prev_frame = None
        total_steps = ((end_frame - start_frame) // check_frame_interval) + 1

        with tqdm(total=total_steps, desc="Checking Frames") as pbar:
            ...
            while frame_number < end_frame:
                ...
                if prev_frame is not None:
                    diff_sum, _ = detect_motion(prev_frame, frame, motion_threshold)
                    # Perform your frame processing here if required
                    pbar.update(1)

                prev_frame = frame
                frame_number += check_frame_interval
                cap.set(cv2.CAP_PROP_POS_FRAMES, frame_number)

        cap.release()
        end_processing_time = time.time()
        print(f"Processing time: {end_processing_time - start_processing_time:.2f} seconds")
        return

    try:
        detected_segments = []
        prev_frame = None
        diff_buffer = []  # Buffer for smoothing
        buffer_size = int(fps)  # 1 second window size in frames
