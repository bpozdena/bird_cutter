...
def parse_arguments():
    ...
    parser.add_argument('--mask', type=str, help='Path to a binary mask image file')
    parser.add_argument('--create_mask', action='store_true', help='Launch GUI to create a mask')
...
def create_mask_gui(video_path):
    def on_mouse(event, x, y, flags, param):
        if event == cv2.EVENT_LBUTTONDOWN:
            points.append((x, y))
            cv2.circle(overlay, (x, y), 3, (0, 255, 0), -1)
            if len(points) > 1:
                cv2.line(overlay, points[-2], points[-1], (0, 255, 0), 2)
            cv2.imshow('Create Mask', cv2.addWeighted(frame, 0.7, overlay, 0.3, 0))
    ...
    cv2.imshow('Create Mask', frame)
    cv2.setMouseCallback('Create Mask', on_mouse)
...
def detect_motion(frame1, frame2, motion_threshold, resize_factor, mask=None):
    if resize_factor != 1:
...
def main():
    ...
    # Handle mask creation or loading
    mask = None
    if args.create_mask:
        mask = create_mask_gui(lowres_video_path)
        cv2.imwrite(os.path.join(output_dir, 'mask.png'), mask)
    elif args.mask:
        mask = cv2.imread(args.mask, cv2.IMREAD_GRAYSCALE)

    if mask is not None:
        print(f"Using mask for motion detection")
        ...
        with tqdm(total=end_frame - start_frame, desc="Processing Frames") as pbar:
            for frame_number in range(start_frame, end_frame):
                ...
                if prev_frame is not None:
                    diff_sum, motion_detected = detect_motion(prev_frame, frame, motion_threshold, resize_factor, mask)
